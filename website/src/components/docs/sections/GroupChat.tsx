...
export default function GroupChat() {
  ...
  return (
    <div>
      ...
      <SubSection title={String(t('/context — Controlling Shared Awareness', '/context — 공유 인식 제어'))}>
        <P>{t(
        ...
        <P>{t(
          <>The <IC>/context</IC> command controls how many of the most recent entries from that shared log are embedded into the bot's system prompt before each turn. This is the mechanism that lets bots "know" what the other bots in the group have recently said and done.</>,
          <><IC>/context</IC> 명령은 각 턴 전에 그 공유 로그에서 가장 최근 몇 개의 항목이 봇의 시스템 프롬프트에 포함될지를 제어합니다. 이것이 봇들이 그룹 내 다른 봇들이 최근에 말하고 한 일을 "알게" 되는 메커니즘입니다.</>
        )}</P>
        <CommandTable
          headers={[String(t('Command', '명령어')), String(t('Description', '설명'))]}
          rows={[
            ...
            ['/context 0', String(t('Disable shared context entirely', '공유 컨텍스트 완전 비활성화'))],
          ]}
        />
        <P>{t(
          <>Default: <strong className="text-zinc-300">12 entries</strong>. Each bot has its own <IC>/context</IC> setting, so you can configure them individually using <IC>@botname /context &lt;n&gt;</IC>.</>,
          <>기본값: <strong className="text-zinc-300">12개 항목</strong>. 각 봇은 자신의 <IC>/context</IC> 설정을 가지므로, <IC>@botname /context &lt;n&gt;</IC>으로 봇별로 개별 설정할 수 있습니다.</>
        )}</P>
        ...
        <P>{t(
          <>Every log entry included via <IC>/context</IC> is embedded into the bot's system prompt on <strong>every turn</strong>. That means:</>,
          <><IC>/context</IC>로 포함된 모든 로그 항목은 <strong>매 턴마다</strong> 봇의 시스템 프롬프트에 삽입됩니다. 이는 다음을 의미합니다:</>
        )}</P>
        <UL>
          <li>{t(
            <>A higher <IC>/context</IC> value → the bot sees more of what other bots are doing → better coordination, but every turn sends more tokens to the AI provider.</>,
            <><IC>/context</IC> 값이 클수록 → 봇이 다른 봇들이 하는 일을 더 많이 볼 수 있고 → 협업이 더 잘 되지만, 매 턴마다 AI 제공자에게 더 많은 토큰을 전송합니다.</>
          )}</li>
          <li>{t(
            <>With multiple bots in the same group all running with a non-zero <IC>/context</IC>, token usage <strong>multiplies</strong>: each bot independently pulls the shared log into its own prompt, so the same conversation content is billed once per bot per turn.</>,
            <>같은 그룹 안에 여러 봇이 모두 0이 아닌 <IC>/context</IC>로 돌아가고 있으면 토큰 사용량이 <strong>배수로 증가</strong>합니다: 각 봇이 독립적으로 공유 로그를 자신의 프롬프트에 끌어오므로, 같은 대화 내용이 봇마다 매 턴 한 번씩 청구됩니다.</>
          )}</li>
          <li>{t(
        ...
        <P>{t(
          <>Tune <IC>/context</IC> based on how much cross-bot awareness you actually need. If the bots rarely need to know what each other are doing, a low value (or <IC>0</IC>) is cheaper and often works just as well.</>,
          <>실제로 얼마나 많은 cross-bot 인식이 필요한지에 따라 <IC>/context</IC>를 조정하세요. 봇들이 서로가 하는 일을 거의 알 필요가 없다면, 낮은 값(또는 <IC>0</IC>)이 더 저렴하고 종종 똑같이 잘 동작합니다.</>
        )}</P>

        <h3 className="text-lg font-semibold text-white mt-6 mb-3">
          {t('When to use /context 0', '/context 0을 사용해야 할 때')}
        </h3>
        <P>{t(
          <>Setting <IC>/context</IC> to <IC>0</IC> disables shared context entirely. The bot will have no visibility into what other bots in the group have said — it behaves as if it were alone in the chat, even though other bots remain present.</>,
          <><IC>/context</IC>를 <IC>0</IC>으로 설정하면 공유 컨텍스트가 완전히 비활성화됩니다. 봇은 그룹의 다른 봇들이 무엇을 말했는지 전혀 볼 수 없게 되고 — 다른 봇들이 여전히 그룹에 있더라도 자신이 채팅에 혼자 있는 것처럼 동작합니다.</>
        )}</P>
        <InfoBox type="tip">
          {t(
            <><strong>If you want to use only a single bot in a group chat, always run <IC>/context 0</IC> on that bot.</strong> There is no other bot to coordinate with, so the shared log would only add useless tokens to every prompt. Turning it off removes that overhead completely and keeps each turn as cheap as a plain 1:1 chat.</>,
            <><strong>그룹 채팅에서 봇 하나만 사용하고 싶다면, 그 봇에 대해 반드시 <IC>/context 0</IC>을 실행하세요.</strong> 조율할 다른 봇이 없으므로 공유 로그는 매 프롬프트에 쓸모없는 토큰만 추가할 뿐입니다. 이를 꺼 두면 해당 오버헤드가 완전히 사라지고, 각 턴이 일반 1:1 채팅만큼 저렴하게 유지됩니다.</>
          )}
        </InfoBox>
        <P>{t(
          <><IC>/context 0</IC> is also the right choice when you deliberately want multiple bots to work independently in the same group without influencing each other.</>,
          <><IC>/context 0</IC>은 같은 그룹 내 여러 봇이 서로 영향을 주지 않고 의도적으로 독립적으로 일하기를 원할 때도 맞는 선택입니다.</>
        )}</P>
      </SubSection>
